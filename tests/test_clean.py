...
def test_replace_urls():
    ...
    for text, proc_text in texts:
        assert phi.replace_urls(text, "*URL*") == proc_text
...
def test_replace_emails():
    ...
    assert phi.replace_emails(text, "*EMAIL*") == proc_text


def test_email_addresses():
    for x in email_addresses:
        assert phi.replace_emails(x, "*EMAIL*") == "*EMAIL*"
...
def test_replace_phone_numbers():
    for x in phone_numbers:
        x_phone = phi.replace_phone_numbers(x, "*PHONE*")
...
def test_replace_numbers():
    ...
    assert phi.replace_numbers(text, "*NUM*") == proc_text
...
def test_remove_punct():
    ...
    assert phi.remove_punct(text) == proc_text
...
def test_replace_currency_symbols():
    ...
    for text, proc_text1, proc_text2 in tests:
        assert phi.replace_currency_symbols(text, replace_with=None) == proc_text1
        assert (
            phi.replace_currency_symbols(text, replace_with="*CUR* ")
            == proc_text2
        )
...
def test_fix_bad_unicode():
    ...
    assert phi.fix_bad_unicode(text) == "and install a 'new' society in their"

    assert "všetko" == phi.fix_bad_unicode("všetko")
    assert "Všetko" == phi.fix_bad_unicode("Všetko")
...
def test_to_ascii():
    assert phi.to_ascii_unicode("whatëver") == "whatever"
    assert phi.to_ascii_unicode("Äpfel»", lang="de") == 'Äpfel"'
    assert phi.to_ascii_unicode("Äpfel»", lang="DE") == 'Äpfel"'
...
def test_keep_emojis():
    assert phi.clean(emoji_line) == emoji_line
